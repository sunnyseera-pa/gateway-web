...
describe('<CollectionPage /> rendering', () => {
	...
	it('renders with correct collection information and 1 tool on the "Tools" tab', async done => {
		wrapper = mount(<CollectionPage userState={userStateData.userState} match={match} />);
									...
									await act(async () => {
										...
										toolRequest
											...
											.then(async () => {
												...
												expect(wrapper.find('[data-testid="collectionKeyword0"]').exists()).toEqual(true);
												expect(wrapper.find('[data-testid="collectionKeyword0"]').text().trim()).toEqual('test');
												expect(wrapper.find('[data-testid="collectionKeyword1"]').exists()).toEqual(true);
												expect(wrapper.find('[data-testid="collectionKeyword1"]').text().trim()).toEqual('keyword');
												expect(wrapper.find('[data-testid="privateBadge"]').exists()).toEqual(true);
												expect(wrapper.find('[data-testid="publicBadge"]').exists()).toEqual(false);
												expect(wrapper.find('[data-testid="privateBadge"]').text().trim()).toEqual('Private');
