...
class DatasetOnboarding extends Component {
    ...
    async initPage() {
        try {
            ...
            if (
                !_.isEmpty(this.state.structuralMetadata) &&
                _.isEmpty(this.state.structuralMetadataErrors)
            )
    ...
    setScreenData = async (context) => {
        ...
        if (
            ...
            applicationStatus === DatasetOnboardingHelper.datasetStatus.inReview
        )
            userType = 'ADMIN';
        ...
        let isThereALiveVersion =
            listOfDatasets.filter((x) => x.activeflag === 'active').length > 0 ? true : false;
    ...
    injectObservations(jsonSchema = {}, questionAnswers = {}) {
        ...
        listOfObservationFields.forEach((field) => {
            Object.keys(questionAnswers).some(function (key) {
                ...
                if (key.toLowerCase().match(regex)) {
                    ...
                    if (
                        !_.isEmpty(uniqueId) &&
                        !listOfObservationUniqueIds.find((x) => x === uniqueId)
                    ) {
        ...
        listOfObservationUniqueIds.forEach((uniqueId) => {
            let duplicateQuestionSet = DatasetOnboardingHelperUtil.questionSetToDuplicate(
                'add-observations',
                { ...jsonSchema },
                uniqueId
            );
            jsonSchema = DatasetOnboardingHelperUtil.insertSchemaUpdates(
                'add-observations',
    ...
    injectStaticContent(jsonSchema = {}, inReviewMode = false, reviewSections = []) {
        ...
        let navElementsExist =
            [...pages].find(
                (page) => page.pageId === DatasetOnboardingHelper.darStaticPageIds.BEFOREYOUBEGIN
            ) || false;
        ...
        if (!navElementsExist) {
            ...
            jsonSchema.formPanels.unshift(
                DatasetOnboardingHelper.staticContent.beforeYouBeginPanel
            );
    ...
    onFormSubmit = async () => {
        ...
        let validationSectionMessages = DatasetOnboardingValidation.buildInvalidSectionMessages(
            Winterfell,
            invalidQuestions
        );
        let inValidMessages = DatasetOnboardingValidation.buildInvalidMessages(
            Winterfell,
            invalidQuestions
        );
        let errors = DatasetOnboardingValidation.formatValidationObj(inValidMessages, [
    ...
    updateApplication = async (obj = {}) => {
        try {
            ...
            const response = await axios.patch(
                `${baseURL}/api/v1/dataset-onboarding/${id}`,
                params
            );
            // 6. Get saved time
            const lastSaved = DatasetOnboardingHelper.saveTime();
        ...
        if (!_.isEmpty(questionSet) && !_.isEmpty(questionId)) {
            ...
            switch (action) {
                ...
                case 'removeObservation':
                    jsonSchema = DatasetOnboardingHelperUtil.removeQuestionReferences(
                        questionSetId,
                        questionId,
                        { ...schema }
                    );
                    questionAnswers = DatasetOnboardingHelperUtil.removeQuestionAnswers(
                        questionId,
                        { ...this.state.questionAnswers }
                    );
    ...
    getActiveQuestionGuidance(questionId = '') {
        ...
        if (!_.isEmpty(questionId)) {
            ...
            let questionList =
                [...questionSets].filter((q) =>
                    q.questionSetId.includes(this.state.activePanelId)
                ) || [];
            questions = questionList.map(({ questions }) => questions).flat();
            if (!_.isEmpty(questions)) {
                // 2. loop over and find active question
                let activeQuestion = DatasetOnboardingHelper.getActiveQuestion(
                    [...questions],
                    questionId
                );
    ...
    postQuestionAction = async (questionSetId, questionId, mode) => {
        let response = await axios.post(
            `${baseURL}/api/v1/data-access-request/${this.state._id}/amendments`,
            {
                questionSetId,
                questionId,
                mode,
    ...
    onStructuralMetaDataUpdate = (structuralMetadata, structuralMetadataErrors) => {
        let percentageCompleted = DatasetOnboardingHelperUtil.getCompletionPercentages(this);
        if (!_.isEmpty(structuralMetadata) && _.isEmpty(structuralMetadataErrors))
            percentageCompleted.updatedCompletion.structural = 100;
        else percentageCompleted.updatedCompletion.structural = 0;
        this.setState({
            structuralMetadata,
        ...
        switch (type) {
                ...
                let alert = {
                    ...
                    message:
                        type !== 'CONFIRMREJECTION'
                            ? 'You have approved the dataset.'
                            : 'You have rejected the dataset.',
                };
            ...
            case 'UNARCHIVE':
                try {
                    ...
                    const response = await axios.put(
                        `${baseURL}/api/v1/dataset-onboarding/${_id}`,
                        body
                    );
                } catch (err) {
                    console.log(err);
            ...
            case 'DELETEDRAFT':
                try {
                    ...
                    await axios
                        .delete(`${baseURL}/api/v1/dataset-onboarding/delete/${id}`)
                        .then((res) => {
            ...
            case 'DUPLICATE':
                try {
                    ...
                    await axios
                        .post(`${baseURL}/api/v1/dataset-onboarding/duplicate/${_id}`)
                        .then((res) => {
            ...
            if (!_.isEmpty(questionAnswers)) {
                ...
                fullAmendments = _.reduce(
                    ...
                    (obj, value, key) => {
                        ...
                        let activeQuestionPanel = [...questionPanels].find(
                            (panel) => panel.panelId === questionSetId
                        );
    ...
    checkUniqueTitle = async (title) => {
        let result = await axios.get(
            `${baseURL}/api/v1/dataset-onboarding/checkUniqueTitle?pid=${this.state.dataset.pid}&&title=${title}`
        );
    ...
    render() {
        ...
        Winterfell.validation.default.addValidationMethods({
            ...
            isValidDoiName: (value) => {
                return (
                    _.isEmpty(value) ||
                    !!value.match(/\b(10[.][0-9]{4,}(?:[.][0-9]+)*\/(?:(?!["&\'<>])\S)+)\b/gm)
                );
            },
            ...
            isMultiFieldURL: (value) => {
                ...
                let isNoError = true;
                value.forEach((url) => {
                    if (!_.isEmpty(url) && !url.match(/^(?:[a-z][a-z0-9+\-.]*:)(?:\/?\/)?[^\s]*$/i))
                        isNoError = false;
                });
                return isNoError;
            },
            isAgeRangeValid: (value) => {
                return (
                    _.isEmpty(value) ||
                    !!value.match(
                        /(150|1[0-4][0-9]|[0-9]|[1-8][0-9]|9[0-9])-(150|1[0-4][0-9]|[0-9]|[1-8][0-9]|9[0-9])/i
                    )
                );
            },
            ...
            <div>
                ...
                <Row className="banner">
                    <Col sm={12} md={8} className="banner-left">
                        ...
                        <span
                            className="white-16-semibold pr-5"
                            ...
                            <Dropdown>
                                ...
                                <Dropdown.Menu as={CustomMenu} className="listOfVersionsDropdown">
                                    {listOfDatasets.map((dat) => {
                                        return (
                                            <Dropdown.Item
                                                href={`/dataset-onboarding/${dat._id}`}
                                                className="black-14"
                                            >
                                                {dat.datasetVersion}
                    ...
                    >
                        <span className="white-14-semibold">
                            {DatasetOnboardingHelper.getSavedAgo(lastSaved)}
                        </span>
                        {
                            <a
                ...
                <div id="darContainer" className="flex-form">
                                    ...
                                    >
                                        <div>
                                            ...
                                            {(() => {
                                                ...
                                                [...this.state.jsonSchema.questionPanels].map(
                                                    (item2, index) => {
                                                        if (
                                            ...
                                            <div>
                                                ...
                                                {item.flag && (
                                                    <i
                                                        className={
                                                            DatasetOnboardingHelper.flagIcons[
                                                                item.flag
                                                            ]
                                                        }
                                                    />
                                    ...
                                    {item.active && (
                                        <ul className="list-unstyled section-subheader">
                                            <NavItem
                                                ...
                                                notForReview={
                                                    !item.inReview && this.state.inReviewMode
                                                }
                    ...
                    <div
                        id="darCenterCol"
                        className={isWideForm ? 'extended' : '' || isTableForm ? 'table' : ''}
                    >
                        ...
                        <div
                            className={`dar__questions ${
                                this.state.activePanelId === 'beforeYouBegin' ? 'pad-bottom-0' : ''
                            }
                ...
                <ActionBar userState={userState}>
                    <div className="action-bar">
                        <div className="action-bar--questions">
                            <SLA
                                classProperty={
                                    DatasetOnboardingHelper.datasetStatusColours[applicationStatus]
                                }
