...
class DataAccessRequestPOC extends Component {
    ...
    updateNavigation = (newForm) => {
        ...
        const currentActivePage = [...pages].find(p => p.active === true);
        if(currentActivePage.pageId !== newForm.pageId) {
            ...
            if(newPageindex > -1) {
                ...
                const { panelId = '' } = [...this.state.form.formPanels].find(p => p.pageId === newFormState[newPageindex].pageId) || {panelId: ''};
                if (!isEmpty(panelId) || panelId != undefined) {
