...
const AddEditWorkflow = (props) => {
  ...
  const deletePhase = (values) => {
    let {steps} = {...values};
    if(!_.isEmpty(steps)) {
      let newSteps = [...steps].filter((key, idx) => { return idx !== phaseIndex});
      setPhaseIndex(-1);
      return {
        ...values,
        steps: newSteps
  ...
  const handleFormSubmission = async () => {
    let {current: { values, setSubmitting }} = formRef;
    ...
    if(existingWorkflow) {
      ...
      const {data: { workflow }} = await axios.post(`${baseURL}/api/v1/workflows`, params);
            ...
            steps: Yup.array()
              .of(
                Yup.object().shape({
                  ...
                  reviewers: Yup.array()
                    .of(Yup.string())
                    .required('You must select at least one reviewer'),
                  sections: Yup.array()
                    .of(Yup.string())
            ...
            <div className="row justify-content-md-center">
              <div className="col-sm-12 col-md-10">
                    ...
                    <div className="main-header-desc">
                      <div>
                        { 
                          editWorkflowName ? 
                          <Fragment>
                            <Field name="workflowName" type="text" className={`form-control ${errors.workflowName && touched.workflowName ? 'is-invalid' : ''}`} />
                            {errors.workflowName && touched.workflowName ? <div className="errorMessages">{errors.workflowName}</div> : null}
                          </Fragment>

                          : <h1 className="black-20-semibold">{values.workflowName}</h1> 
                        }
                      </div>
                      <div className="soft-black-14">Workflows are the frameworks used by your team to respond to data access request applications.</div>
                    </div>{/* CLOSE HEADER-DESCRIPTION */}
                    <div className="main-header-action">
                      <button className="button-tertiary mr-2" type="button" onClick={e => onClickWorkFlowName(e)}>{editWorkflowName ? 'Save name' : 'Edit name'}</button>
                      <button className="button-tertiary" type="button" onClick={e => toggleModal(modalactions.DELETEWORKFLOW)}>Delete</button>
                    </div>{/* CLOSE MAIN-HEADER-ACTION */}
                      ...
                      render={({ push }) => (
                        <div>
                              ...
                              return (
                                <div key={`step-${index}`} className="main-accordion">
                                  <div className="main-accordion-header" onClick={(e) => {
                                    ...
                                    <h1>{index + 1}. {node.stepName}</h1>                              
                                  </div>
                                  <SlideDown closed={node.expand}>
                                    <div className="main-accordion-body">
                                      <div className="form-group">
                                        <label htmlFor={`steps[${index}].stepName`} className="form-label">Phase Name</label>
                                        <Field 
                                          type="text" 
                                          name={`steps[${index}].stepName`} 
                                          id={`steps[${index}].stepName`} 
                                          className={`form-control control-name ${hasErrors(touched, errors, index, 'stepName') ? 'is-invalid' : ''}`}  />
                                          {hasErrors(touched, errors, index, 'stepName') ? <div className="errorMessages">{errors.steps[index].stepName}</div> : null }
                                      </div>
                                      <div className="form-group">
                                        <label htmlFor={`steps[${index}].reviewers`} className="form-label">Reviewers</label>
                                        <small className="form-text mb-2">Only members of your team can be added as reviewers. To add members to your team email members@HDRUK.org.</small>
                                        <TypaheadMultiUser
                                          apiCall='teams'
                                          team={team}
                                      ...
                                      <div className="form-group">
                                        <label htmlFor={`node.${index}.sections`} className="form-label">What sections of the application can be reviewed?</label>
                                        <small className="form-text mb-2">Reviewers can see all sections of the application, but can only interact with and review their assigned sections</small>
                                        ...
                                        {hasErrors(touched, errors, index, 'sections') ? <div className="errorMessages">{errors.steps[index].sections}</div> : null }
                                      </div>
                                      <div className="form-group">
                                        <label htmlFor={`node[${index}].start`} className="form-label">When will this phase start and end?</label>
                                        <small className="form-text">The first phase will begin once the workflow manager has started the review. The following phases begin once a decision has been made by all reviewers on the previous phase, or once the manager has progressed the review to the next phase. Reviewers will be notified when their assigned phase has begun.</small>
                                      </div>
                                      <div className="form-group">
                                        ...
                                        <div className="form-inline">
                                          <Field 
                                            ...
                                            className={`form-control control-deadline ${hasErrors(touched, errors, index, 'deadline') ? 'is-invalid' : ''}`} />
                                          <small className="ml-2">days after the start of this phase</small>
                                        </div>
                                        {hasErrors(touched, errors, index, 'deadline') ? <div className="errorMessages">{errors.steps[index].deadline}</div> : null }
                                      </div>
                                      <div className="form-group phase-action">
                                        ...
                                        <CloseButtonSvg width="10px" height="10px" fill="#475DA7" /> Remove phase</button>
                                      </div>
                                    </div>
                                  </SlideDown>
                                </div>
                          ...
                          <div className="main-footer">
                            <button type="button" className="button-secondary" onClick={e => push(step)}>+ Add another phase</button>
                          </div>{/* CLOSE FOOTER */}
                        </div>
                      )} 
                    />{/* CLOSE FIELD-ARRAY */}
                ...
                <div className="workflow-action-bar">
                  <button className="button-tertiary" onClick={e => returnToWorkflows(e)}>Return to all workflows</button>
                  <button className={savedSuccess ? 'button-teal' : 'button-primary'} type="submit" disabled={isSubmitting}> {savedSuccess ? <div><CheckSVG fill="#fff" className="submitClose" /> Saved</div> : 'Save'}</button>
                </div>
              </div>
            </div>
          </Form>
