        ...
        if(this.accountMembersId){
            ...
            await axios.get(baseURL + `/api/v1/teams/${this.accountMembersId}/members`)
                .then(async(res) => {
                    ...
                    this.setState({userIsManager: 
                        res.data.members
                            .filter((m) => m.id === this.state.userState[0].id)
                            .map(m => m.roles[0] === "manager")[0]
                    });
            })
        }
                    ...
                    <Col xs={10}>
                        <Row className="accountHeader">
                            <Col sm={12} md={8}>
                                ...
                                <Row>
                                    <span className="gray700-13">To remove team members or change their roles, please raise a support ticket at the following link:</span>
                                </Row>
                                <Row>
                                    <span className="purple-13 pad-bottom-24"> <a href="https://hdruk.atlassian.net/servicedesk/customer/portal/1">https://hdruk.atlassian.net/servicedesk/customer/portal/1</a></span>
                                </Row>
                                <Row>
                                    <span className="gray700-13 pad-bottom-24">Managers can; manage members, create and assign workflows, review applications that are assigned to them and make the final decision on data access request applications.</span>
                                </Row>
                                <Row>
                                    ...
                                    : members.map((m) => {
                                        return (
                                            <Row className="entryBox padding-left-20 ">
                                                <Col sm={12} lg={5}><a href={'/person/' + m.id} className="purple-14">{m.firstname} {m.lastname}</a>
                                                    <Row sm={5} lg={5} >
                                                         <Col sm={10} lg={10} className="gray-600-14 ellipsis">{m.organisation ? m.organisation : m.bio}</Col>
                                                    </Row>
                                                </Col>
                                                <Col sm={4} lg={4} className="black-14">{m.roles[0].charAt(0).toUpperCase() + m.roles[0].slice(1)}</Col>
                                            </Row>
                                        )
                                    })}
