 ...
 * ]
 */
const createCSV = async dataAccessRequests => {
	try {
		// set headers
		...
		for (const dar of dataAccessRequests) {
			const rows = await buildRows(dar);
			csvRows = await csvRows.concat(rows);
		}
...
const buildRows = async dar => {
	...
	for (const applicant of applicants) {
		const row = await buildOneRow(dar, applicant);
		rows.push(row);
	}
...
const FetchKeyNameBeforeTheUnderScore = key => {
	if (key.indexOf('_') > -1) return key.substring(0, key.indexOf('_'));
	else return key;
};
